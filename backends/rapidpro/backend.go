...
func (b *backend) NewOutgoingMsg(channel courier.Channel, id courier.MsgID, urn urns.URN, text string, highPriority bool, quickReplies []string, topic string, responseToExternalID string) courier.Msg {
...
func (b *backend) WriteMsg(ctx context.Context, m courier.Msg) error {
	...
	optOutDisabled := m.Channel().OrgConfigForKey(utils.OptOutDisabled, false)
	if handlers.CheckChannelOptOut(m) && optOutDisabled == false {
